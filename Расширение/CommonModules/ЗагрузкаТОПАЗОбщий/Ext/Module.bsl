...
// Используется регламентным заданием для вызова метода обработки
//
// Параметры:
//  ДеньЗагрузки - Дата - день загрузки данных из XML файлов ТОПАЗ
//
Процедура ВыполнитьЗагрузкуДанныхИзТОПАЗ(Знач ДеньЗагрузки = Неопределено) Экспорт
	
	Обработка = Обработки.ЗагрузкаДанныхИзТОПАЗ.Создать();
	Обработка.ВыполнитьЗагрузку(ДеньЗагрузки);
...
// Находит номенклатуру по коду (штрихкоду)
//
// Параметры:
//  Код - Строка - код (штрихкод) номенклатуры
//
// Возвращаемое значение:
//  СправочникСсылка.Номенклатура - ссылка на номенклатуру, или Неопределено если не найдена
//
...
Функция ОпределитьСпособОплаты(AmountCash, AmountCashless) Экспорт
	...
	Если AmountCash > 0 И AmountCashless > 0 Тогда
		...
		Результат.Вставить("СпособОплаты", Перечисления.ТипыОплатыККТ.Наличные);
		Результат.Вставить("Сумма", AmountCash);
		Результат.Вставить("СпособОплаты2", Перечисления.ТипыОплатыККТ.Электронно);
		Результат.Вставить("Сумма2", AmountCashless);
	ИначеЕсли AmountCash > 0 Тогда
		Результат.Вставить("СпособОплаты", Перечисления.ТипыОплатыККТ.Наличные);
		Результат.Вставить("Сумма", AmountCash);
	ИначеЕсли AmountCashless > 0 Тогда
		Результат.Вставить("СпособОплаты", Перечисления.ТипыОплатыККТ.Электронно);
