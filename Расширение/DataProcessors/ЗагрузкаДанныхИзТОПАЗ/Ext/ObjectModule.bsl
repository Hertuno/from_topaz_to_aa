...
//
Процедура ВыполнитьЗагрузку(ДеньЗагрузки = Неопределено) Экспорт
	...
	Попытка
		...
		МаскаФайла = СтрШаблон("CloseSession_%1-%2-%3*.xml", Год(ТекущаяДата), Месяц(ТекущаяДата), День(ТекущаяДата));
...
Функция ИзвлечьДанныеСессии(ПутьКФайлу)
	...
	Попытка
		...
		ЧтениеXML = ПрочитатьXMLИзФайла(ПутьКФайлу);
