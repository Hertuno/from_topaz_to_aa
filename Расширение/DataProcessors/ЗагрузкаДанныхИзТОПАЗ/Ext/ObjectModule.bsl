	...
	Попытка
		...
		Для Каждого ПутьКФайлу Из НайденныеФайлы Цикл
			...
			Попытка
				...
				ОбработатьФайл(ПутьКФайлу, Настройки, РучнойЗапуск);
				ОбработанныеФайлы = ОбработанныеФайлы + 1;
				Комментарий = "Файл успешно обработан: " + ПутьКФайлу;

				РегистрацияПрогресса(ИмяСобытия, "Информация", Комментарий);
				
			Исключение
				ТекстОшибки = МенеджерОбработкиОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				Комментарий = СтрШаблон("Ошибка при обработке файла %1: %2", ПутьКФайлу, ТекстОшибки);
...
//  РучнойЗапуск - Булево - ручной запуск загрузки
//
Процедура ОбработатьФайл(ПутьКФайлу, Настройки, РучнойЗапуск)
	...
	Попытка
		...
		ДатаДокумента = КонецДня(ДанныеСессии.ДатаФайла);
		...
		Для Каждого Операция Из ОперацииРозницы Цикл
			...
			Исключение
				ТекстОшибки = МенеджерОбработкиОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				Комментарий = "Ошибка при создании документов розницы: " + ТекстОшибки;
				РегистрацияПрогресса(ИмяСобытия, "Ошибка", Комментарий, РучнойЗапуск);
			КонецПопытки;
	...
	Исключение
		...
		ТекстОшибки = МенеджерОбработкиОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Комментарий = "Критическая ошибка при обработке файла " + ПутьКФайлу + ": " + ТекстОшибки;
		РегистрацияПрогресса(ИмяСобытия, "Ошибка", Комментарий, РучнойЗапуск);
...
Функция ПрочитатьИзФайла(ПутьКФайлу)
	...
	Попытка
		...
		ЧтениеТекста.Открыть(ПутьКФайлу, , "windows-1251");
...
Функция ИзвлечьДанныеСессии(ПутьКФайлу)
	...
	Попытка
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если НЕ (ЧтениеXML.ИмяЭлемента = "DataPaket" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента) Тогда
				Прервать;
			КонецЕсли;
...
Процедура РегистрацияПрогресса(Знач Событие, Знач Уровень, Знач Комментарий = "", Знач ВыводСообщений = Ложь)
	...
	Если ВыводСообщений Тогда
		...
		Сообщение.Текст = СтрШаблон("%1: %2", Уровень, Комментарий);
		Сообщение.Сообщить();
	Иначе
